...
st.markdown("""
    <div style='text-align: left; font-size: 22px; padding-top: 10px; color: #000000;'>
        Select a county and see the forecasted EV adoption trend for the next 4 years.
    </div>
""", unsafe_allow_html=True)
...
ax.set_title(f"Cumulative EV Trend - {county} (4 Years Forecast)", fontsize=14, color='white')
...
if historical_total > 0:
    ...
    st.success(f"Based on the graph, EV adoption in **{county}** is expected to show a **{trend} of {forecast_growth_pct:.2f}%** over the next 4 years.")
else:
...
if multi_counties:
    ...
    ax.set_title("EV Adoption Trends: Historical + 4-Year Forecast", fontsize=16, color='white')
    ax.set_xlabel("Date", color='white')
    ax.set_ylabel("Cumulative EV Count", color='white')
    ax.grid(True, alpha=0.3)
    ax.set_facecolor("#1c1c1c")
    ...
    st.success(f"Forecasted EV adoption growth over next 4 years — {growth_sentence}")

st.success("**Forecast completed**")

st.markdown("""
    <div style='text-align: center; font-size: 24px; font-weight: bold; padding-top: 10px; margin-bottom: 20px; color: #FFFFFF;'>
